...
int cmdq_core_get_thread_index_from_scenario_and_secure_data(
	...
	const bool secure)
{
	...
	case CMDQ_SCENARIO_SUB_ALL:
	case CMDQ_SCENARIO_MHL_DISP:
		/* because mirror mode and sub disp never
		 * use at the same time in secure path,
...
enum CMDQ_HW_THREAD_PRIORITY_ENUM cmdq_core_priority_from_scenario(
	enum CMDQ_SCENARIO_ENUM scenario)
{
	switch (scenario) {
	case CMDQ_SCENARIO_PRIMARY_DISP:
...
const char *cmdq_core_parse_error_module_by_hwflag_impl(
...
int cmdq_core_dump_smi(const int showSmiDump)
{
	...
	CMDQ_ERR("[WARNING]CMDQ dump SMI now\n");
...
void cmdq_core_dump_secure_metadata(struct cmdqSecDataStruct *pSecData)
{
	...
	CMDQ_LOG("metaData cnt:%d(%d), NeedDAPC:0x%llx, PortSecurity:0x%llx\n",
		 pSecData->addrMetadataCount,
		 pSecData->addrMetadataMaxCount,
	...
	for (i = 0; i < pSecData->addrMetadataCount; i++) {
		CMDQ_MSG("meta idx:%d, inst idx:%d, type:%d, baseHandle:%x\n",
		     i,
		     pAddr[i].instrIndex,
		     pAddr[i].type,
		     pAddr[i].baseHandle);
		CMDQ_MSG("offset:0x%08x, size:%d, port:%d\n",
		     pAddr[i].offset,
		     pAddr[i].size,
		     pAddr[i].port);
	}
