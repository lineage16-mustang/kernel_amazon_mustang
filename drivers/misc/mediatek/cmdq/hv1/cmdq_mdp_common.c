			     ...
			     const char *name, const int32_t maxLoopCount)
{
	...
	while (loop < maxLoopCount) {
		uint32_t resetStateValue =
			(uint32_t)(CMDQ_REG_GET32(resetStateReg) & resetMask);
		if (resetPollingValue == resetStateValue)
			break;
		else
	...
	if (loop >= maxLoopCount) {
		CMDQ_ERR
			("%s fail,Reg:0x%lx,writeValue:0x%x,stateReg:0x%lx\n",
		     __func__,
		     resetReg,
		     resetWriteValue,
		     resetStateReg);
		...
		CMDQ_ERR
		    ("%s failed, mask:0x%08x, pollingValue:0x%08x\n",
		     __func__,
		     resetMask,
		     resetPollingValue);
		return -EFAULT;
			...
			const bool pollInitResult)
{
	...
	if (cmdq_core_clock_is_on(clkId)) {
		...
		/* loop reset */
		resetStatus = cmdq_mdp_loop_reset_impl(resetReg,
							0x1,
							resetStateReg,
							resetMask,
							resetValue,
		...
		if (pollInitResult) {
			/* loop  init */
			initStatus = cmdq_mdp_loop_reset_impl(resetReg,
							0x0,
							resetStateReg,
							resetMask,
							0x0,
		...
		if ((resetStatus < 0) || (initStatus < 0)) {
			CMDQ_ERR("Reset MDP %s failed, reset:%d, init:%d\n",
				 name,
				 resetStatus,
				 initStatus);
			return -EFAULT;
		       ...
		       const bool pollInitResult)
{
	...
	if (cmdq_core_clock_is_on(clkId)) {
		...
		/* retrun failed if loop failed */
		if ((resetStatus < 0) || (initStatus < 0)) {
			CMDQ_ERR("Disable %s failed, reset:%d, init:%d\n",
				 name,
				 resetStatus,
				 initStatus);
			CMDQ_AEE("MDP",
				"Disable %s failed, reset:%d, init:%d\n",
				 name,
				 resetStatus,
				 initStatus);
			return;
...
void cmdq_mdp_dump_rdma(const unsigned long base, const char *label)
{
	...
	CMDQ_ERR("RDMA_SRC_CON: 0x%08x\n",
	     value[0]);
	CMDQ_ERR("RDMA_SRC_BASE_0: 0x%08x\n",
	     value[1]);
	CMDQ_ERR("RDMA_MF_BKGD_SIZE_IN_BYTE: 0x%08x\n",
	     value[2]);

	CMDQ_ERR("RDMA_MF_SRC_SIZE: 0x%08x\n",
		 value[3]);
	CMDQ_ERR("RDMA_MF_CLIP_SIZE: 0x%08x\n",
		 value[4]);
	CMDQ_ERR("RDMA_MF_OFFSET_1: 0x%08x\n",
		 value[5]);

	CMDQ_ERR("RDMA_SRC_END_0: 0x%08x\n",
		 value[6]);
	CMDQ_ERR("RDMA_SRC_OFFSET_0: 0x%08x\n",
		 value[7]);
	CMDQ_ERR("RDMA_SRC_OFFSET_W_0: 0x%08x\n",
		 value[8]);

	CMDQ_ERR("RDMA_MON_STA_0: 0x%08x\n",
		 value[9]);
	CMDQ_ERR("RDMA_MON_STA_1: 0x%08x\n",
		 value[10]);
	CMDQ_ERR("RDMA_MON_STA_2: 0x%08x\n",
		 value[11]);

	CMDQ_ERR("RDMA_MON_STA_4: 0x%08x\n",
		 value[12]);
	CMDQ_ERR("RDMA_MON_STA_6: 0x%08x\n",
		 value[13]);
...
void cmdq_mdp_dump_rsz(const unsigned long base, const char *label)
{
	...
	CMDQ_ERR("RSZ_CONTROL: 0x%08x\n",
		 value[0]);
	CMDQ_ERR("RSZ_INPUT_IMAGE: 0x%08x\n",
		 value[1]);
...
void cmdq_mdp_dump_rot(const unsigned long base, const char *label)
{
	...
	CMDQ_ERR("========= [CMDQ] %s Status ========\n",
		label);
	CMDQ_ERR("ROT_CTRL: 0x%08x\n",
		 value[0]);
	CMDQ_ERR("ROT_MAIN_BUF_SIZE: 0x%08x\n",
		 value[1]);
	CMDQ_ERR("ROT_SUB_BUF_SIZE: 0x%08x\n",
		 value[2]);

	CMDQ_ERR("ROT_TAR_SIZE: 0x%08x\n",
		 value[3]);
	CMDQ_ERR("ROT_BASE_ADDR: 0x%08x\n",
		 value[4]);
	CMDQ_ERR("ROT_OFST_ADDR: 0x%08x\n",
		 value[5]);

	CMDQ_ERR("ROT_DMA_PERF: 0x%08x\n",
		 value[6]);
	CMDQ_ERR("ROT_STRIDE: 0x%08x\n",
		 value[7]);
	CMDQ_ERR("ROT_IN_SIZE: 0x%08x\n",
		 value[8]);


	CMDQ_ERR("ROT_EOL: 0x%08x\n",
		 value[9]);
	CMDQ_ERR("ROT_DBUGG_1: 0x%08x\n",
		 value[10]);
	CMDQ_ERR("ROT_DEBUBG_2: 0x%08x\n",
		 value[11]);

	CMDQ_ERR("ROT_DBUGG_3: 0x%08x\n",
		 value[12]);
	CMDQ_ERR("ROT_DBUGG_4: 0x%08x\n",
		 value[13]);
	CMDQ_ERR("ROT_DEBUBG_5: 0x%08x\n",
		 value[14]);

	CMDQ_ERR("ROT_DBUGG_6: 0x%08x\n",
		 value[15]);
	CMDQ_ERR("ROT_DBUGG_7: 0x%08x\n",
		 value[16]);
	CMDQ_ERR("ROT_DEBUBG_8: 0x%08x\n",
		 value[17]);

	CMDQ_ERR("ROT_DBUGG_9: 0x%08x\n",
		 value[18]);
	CMDQ_ERR("ROT_DBUGG_A: 0x%08x\n",
		 value[19]);
	CMDQ_ERR("ROT_DEBUBG_B: 0x%08x\n",
		 value[20]);

	CMDQ_ERR("ROT_DBUGG_C: 0x%08x\n",
		 value[21]);
	CMDQ_ERR("ROT_DBUGG_D: 0x%08x\n",
		 value[22]);
	CMDQ_ERR("ROT_DEBUBG_E: 0x%08x\n",
		 value[23]);

	CMDQ_ERR("ROT_DBUGG_F: 0x%08x\n",
		 value[24]);
	CMDQ_ERR("ROT_DBUGG_10: 0x%08x\n",
		 value[25]);
	CMDQ_ERR("ROT_DEBUBG_11: 0x%08x\n",
		 value[26]);

	CMDQ_ERR("ROT_DEBUG_12: 0x%08x\n",
		 value[27]);
	CMDQ_ERR("ROT_DBUGG_13: 0x%08x\n",
		 value[28]);
...
void cmdq_mdp_dump_wdma(const unsigned long base, const char *label)
{
	...
	CMDQ_ERR("=============== [CMDQ] %s Status =====\n",
		label);
	CMDQ_ERR("[CMDQ]WDMA_CFG: 0x%08x\n",
		 value[0]);
	CMDQ_ERR("[CMDQ]WDMA_SRC_SIZE: 0x%08x\n",
		 value[1]);
	CMDQ_ERR("[CMDQ]WDMA_DST_W_IN_BYTE = 0x%08x\n",
		 value[2]);
	CMDQ_ERR("[CMDQ]WDMA_DST_ADDR0: 0x%08x\n",
	     value[3]);
	CMDQ_ERR("[CMDQ]WDMA_DST_UV_PITCH: 0x%08x\n",
	     value[4]);
	CMDQ_ERR("[CMDQ]WDMA_DST_ADDR_OFFSET0 = 0x%08x\n",
	     value[5]);
