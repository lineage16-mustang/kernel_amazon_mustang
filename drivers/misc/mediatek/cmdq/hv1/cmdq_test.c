...
/* {normal, secure},configData, paraData */
static int32_t gCmdqTestConfig[3] = { 0, 0, 0 };
...
static void testcase_async_request_partial_engine(void)
{
	...
	struct timer_list timers[ARRAY_SIZE(scn)];
...
static void testcase_sync_token_threaded(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(scn); ++i) {
		setup_timer(&eventTimer,
			&_testcase_unlock_all_event_timer_func,
			0);
...
static void testcase_loop(void)
{
	...
	setup_timer(&g_loopTimer,
		&_testcase_loop_timer_func,
		CMDQ_SYNC_TOKEN_USER_0);
	mod_timer(&g_loopTimer, jiffies + msecs_to_jiffies(300));
	CMDQ_REG_SET32(CMDQ_SYNC_TOKEN_UPD, CMDQ_SYNC_TOKEN_USER_0);
...
/* clkmgr is not available on FPGA */
	...
	CMDQ_VERBOSE("write reg:0x%lx to 0x%08x, read reg(0x%lx), result:%d\n",
		testWriteReg,
		testWriteValue,
		testReadReg,
		verifyWriteResult);
	...
	/* turn off CLK, function should not work
	 * and access register should not cause hang
	 */
...
static void testcase_dram_access(void)
{
	...
	*pCmdEnd = (CMDQ_CODE_MOVE << 24) |
	 ...
	 * Read data from *CMDQ_DATA_REG_DEBUG_DST
	 * to CMDQ_DATA_REG_DEBUG
	 */
	*pCmdEnd = CMDQ_DATA_REG_DEBUG;
	pCmdEnd += 1;
	/*1 1 0 */
	*pCmdEnd = (CMDQ_CODE_READ << 24) |
		(0 & 0xffff) |
		((CMDQ_DATA_REG_DEBUG_DST & 0x1f) << 16) |
		(6 << 21);
	pCmdEnd += 1;

	/* Load ddst_addr to GPR: Move &(regResults[1])
...
static void testcase_perisys_apb(void)
{
	...
	const uint32_t AUDIO_TOP_MASK = ~0 &
						~(1 << 28 |
						1 << 21 |
						1 << 17 |
						1 << 16 |
	...
	if (data != dataRead)
		CMDQ_ERR("TEST FAIL: data is 0x%08x, different=====\n",
			dataRead);
...
static void testcase_write_from_data_reg(void)
{
	...
	if (value != PATTERN) {
		CMDQ_ERR("%s failed, dstReg isn't 0x%08x, value: 0x%08x\n",
			__func__,
			PATTERN,
			value);
	}
...
static void testcase_read_to_data_reg(void)
{
	...
	/* move data from GPR to GPR_Px:
	 * COLOR to COLOR_DST (64 bit)
	 */
	cmdqRecReadToDataRegister(handle,
		CMDQ_GPR_R32_PA(CMDQ_DATA_REG_PQ_COLOR),
		CMDQ_DATA_REG_PQ_COLOR_DST);
	/* move data from register value to GPR_Rx:
	 * MM_DUMMY_REG to COLOR(32 bit)
	 */
	cmdqRecReadToDataRegister(handle,
		CMDQ_TEST_DISP_PWM0_DUMMY_PA,
		CMDQ_DATA_REG_PQ_COLOR);
	...
	data = CMDQ_REG_GET32(CMDQ_GPR_R32(CMDQ_DATA_REG_PQ_COLOR));
	if (data != 0xdeaddead)
		CMDQ_ERR("TEST FAIL: PQ reg value is 0x%08x\n",
			data);
	...
	if (data64 != 0xbeefbeef)
		CMDQ_ERR("TEST FAIL: PQ_DST reg value is 0x%llx\n",
			data64);
...
static void testcase_write_reg_from_slot(void)
{
	...
	cmdqRecBackupWriteRegisterFromSlot(handle,
		hSlot,
		0,
		CMDQ_TEST_DISP_PWM0_DUMMY_PA);
...
static void testcase_update_value_to_slot(void)
{
	...
	for (i = 0; i < 5; ++i) {
		...
		if (PATTERNS[i] != value) {
			CMDQ_ERR("slot[%d] = 0x%08x. should be 0x%08x\n",
				 i, value, PATTERNS[i]);
		} else {
...
static void testcase_poll(void)
{
	 ...
	 * sw timeout will be hanppened before CPU schedule
	 * to set value..., so we set value here
	 */
...
/* start a thread to poll CMDQ_TEST_VA register */
	struct task_struct *pThread = kthread_run(_thread_poll_reg_value,
				NULL,
				"cmq_thread_poll");
...
int32_t callback_func(unsigned long data)
{
	...
	case WAIT_OVL0_EOF_NORMAL:
		CMDQ_LOG("testcase_wait_ovl0_eof execute done in normal\n");
		break;
	case WAIT_OVL0_EOF_SECURE:
		CMDQ_LOG("testcase_wait_ovl0_eof execute done in secure\n");
		break;

	case WAIT_RDMA0_EOF_NORMAL:
		CMDQ_LOG("testcase_wait_rdma0_eof execute done in normal\n");
		break;
	case WAIT_RDMA0_EOF_SECURE:
		CMDQ_LOG("testcase_wait_rdma0_eof execute done in secure\n");
		break;

	case WAIT_MUTEX0_EOF_NORMAL:
		CMDQ_LOG("wait_mutex0_eof execute done in normal\n");
		break;
	case WAIT_MUTEX0_EOF_SECURE:
		CMDQ_LOG("wait_mutex0eof execute done in secure\n");
		break;
	default:
...
static void testcase_wait_disp_rdma0_eof(void)
{
	...
	cmdqRecFlushAsyncCallback(handle, callback_func,
		gCmdqTestSecure ?
		WAIT_RDMA0_EOF_SECURE :
		WAIT_RDMA0_EOF_NORMAL);
	cmdqRecDestroy(handle);
}
...
static void testcase_wait_disp_ovl0_eof(void)
{
	...
	cmdqRecWait(handle, CMDQ_EVENT_DISP_OVL0_EOF);
	cmdqRecWait(handle, CMDQ_EVENT_DISP_OVL0_EOF);
	cmdqRecFlushAsyncCallback(handle,
		callback_func,
		gCmdqTestSecure ?
		WAIT_OVL0_EOF_SECURE :
		WAIT_OVL0_EOF_NORMAL);
	cmdqRecDestroy(handle);
}
...
static void testcase_dump_error_task_to_file(void)
{
	...
	if (gCmdqContext.errorTask.errTaskCount == 0) {
		CMDQ_ERR("no error task exist\n");
		return;
	}

	for (i = 0; i < gCmdqContext.errorTask.errTaskCount; i++) {
		struct ErrorTaskItemStruct *pErrorTaskBuffer =
			&gCmdqContext.errorTask.errorTaskBuffer[i];
		if (pErrorTaskBuffer->pVABase) {
			CMDQ_LOG("SLOT[%d] pTask[%p] trigger[%lld]\n",
				i,
				pErrorTaskBuffer->pTask,
				pErrorTaskBuffer->trigger);
			CMDQ_LOG("pVABase[%p] commandSize[%d]\n",
				 pErrorTaskBuffer->pVABase,
				 pErrorTaskBuffer->commandSize);
#if 1
			switch (i) {
			case 0:
				cmdq_core_dump_instructions_to_file(
					(uint64_t *) pErrorTaskBuffer->pVABase,
					pErrorTaskBuffer->commandSize,
...
static void testcase_write(void)
{
	...
	value = CMDQ_REG_GET32(CMDQ_TEST_MMSYS_DUMMY_VA);
	if (value != PATTERN)
		CMDQ_ERR("TEST FAIL: wrote is 0x%08x, not 0x%08x\n",
			value,
			PATTERN);
	else
		CMDQ_ERR("TEST OK: wrote is 0x%08x, read is 0x%08x\n",
			value,
			PATTERN);
#endif
}
...
static int _testcase_thread_dispatch(void *data)
{
	...
	_testcase_simplest_command_loop_submit(1000,
		CMDQ_SCENARIO_DEBUG,
		engineFlag,
		false);
...
static void testcase_use_backup_slot_to_debug(void)
{
	...
	if ((gCmdqTestConfig[2] % 2) != 0) {
		CMDQ_LOG("set TESTREG to %d with CPU",
			gCmdqTestConfig[2]);
		CMDQ_REG_SET32(CMDQ_TEST_MMSYS_DUMMY_VA,
			gCmdqTestConfig[2]);
	} else {
		CMDQ_LOG("set TESTREG to %d with CMDQ",
			gCmdqTestConfig[2]);
		cmdqRecWrite(handle,
			CMDQ_TEST_DISP_PWM0_DUMMY_PA,
			gCmdqTestConfig[2],
...
void testcase_read_init_memory(void)
{
	...
	do {
		status = cmdq_sec_submit_to_secure_world_async_unlocked(
				...
				NULL);
		if (status < 0)
			CMDQ_ERR("entry %s failed, status:%d\n",
				__func__,
				status);
...
void testcase_secure_disp_scenario(void)
{
	 ...
	 * otherwise we should disable "enable GCE" in SWd
	 * in order to prevent phone hang
	 */
	...
	if (value != PATTERN)
		...
		CMDQ_LOG("TEST OK: wrote is 0x%08x, read is 0x%08x\n",
			PATTERN,
			value);
...
{
	struct cmdqRecStruct *hTestDISP;
...
static void testcase_DSI_command_mode(void)
{
	 ...
	 * WFE wait stream done (CMDQ_EVENT_MUTEX0_STREAM_EOF?
	 * TODO: which stream done??
	 * and clear  (CMDQ_EVENT_MUTEX0_STREAM_EOF)
	 * WRITE enable mutex to start engine
	 * (DISP_MUTEX0_EN in disp_mutex_coda.xls)
	 * EOC (to issue interrupt, may need callback???
	 * why need this one???)
	...
	/* Jump back to head */
	/* bit 32: is_absolute */
	cmdq_append_command(hSetting, CMDQ_CODE_JUMP, 0,
				-hSetting->blockSize);
	hSetting->priority = 1;


	/* Both are inifinte loop so we call async then sync */
...
void testcase_write_stress_test(void)
{
	...
	loop = 1;
	CMDQ_LOG("=============== loop x %d ===============\n", loop);
	_testcase_simplest_command_loop_submit(loop,
		CMDQ_SCENARIO_DEBUG,
		0,
		gCmdqTestSecure);
...
ssize_t cmdq_test_proc(struct file *fp, char __user *u, size_t s, loff_t *l)
{
	...
	smp_mb();	/*coding style requires to comment */

	testId = gCmdqTestConfig[0];
	isSecureTest = (gCmdqTestConfig[1] > 0) ? (true) : (false);
					...
					loff_t *data)
{
	...
	if (strcmp(desc, "dump_error_task\n") == 0) {
		testcase_dump_error_task_to_file();
		return count;
	}

	if (sscanf(desc, "%d %d %d",
			&testType,
			&newTestSuit,
			&paramData) <= 0) {
