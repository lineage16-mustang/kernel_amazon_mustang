...
int32_t cmdq_rec_realloc_addr_metadata_buffer(struct cmdqRecStruct *handle,
	const uint32_t size)
{
	void *pNewBuf = NULL;
	...
	if (!pNewBuf) {
		CMDQ_ERR("REC: kzalloc %d bytes addr_metadata buffer failed\n",
			 size);
		return -ENOMEM;
	}
	...
	struct cmdqRecStruct **pHandle)
{
	...
	handle = kzalloc(sizeof(uint8_t *) * sizeof(struct cmdqRecStruct),
		GFP_KERNEL);
	if (!handle)
		return -ENOMEM;
	...
	uint32_t argB)
{
	...
	/* be careful that subsys encoding position is
	...
	if (handle->finalized) {
		CMDQ_ERR("Already finalized record 0x%p, cannot add more\n",
			handle);
		return -EBUSY;
	}
	...
	if ((handle->blockSize + 32) >= handle->bufferSize) {
		if (cmdq_rec_realloc_cmd_buffer(handle, handle->bufferSize * 2))
			return -ENOMEM;
	...
	if (code != CMDQ_CODE_EOC &&
		...
		if (handle->prefetchCount >= CMDQ_MAX_PREFETCH_INSTUCTION) {
			CMDQ_MSG("prefetchCount(%d) too large\n",
	...
	CMDQ_VERBOSE("REC: 0x%p CMD: 0x%p, op: 0x%02x\n",
		...
		pCommand,
		code);
	CMDQ_VERBOSE("REC: argA: 0x%08x, argB: 0x%08x\n",
		argA,
		argB);
	...
	case CMDQ_CODE_READ:
		...
		*pCommand++ = argB;
		*pCommand++ = (CMDQ_CODE_READ << 24) |
			(argA & 0xffff) |
			((subsys & 0x1f) << subsysBit) |
			(2 << 21);
		break;
	case CMDQ_CODE_MOVE:
		*pCommand++ = argB;
	...
	case CMDQ_CODE_WRITE:
		subsys = cmdq_subsys_from_phys_addr(argA);
		if (-1 == subsys) {
			CMDQ_ERR("REC: Unsupported memory base 0x%08x\n",
				argA);
			return -EFAULT;
		}

		*pCommand++ = argB;
		*pCommand++ = (CMDQ_CODE_WRITE << 24) |
			(argA & 0x0FFFF) |
			((subsys & 0x01F) << subsysBit);
		break;
	case CMDQ_CODE_POLL:
		subsys = cmdq_subsys_from_phys_addr(argA);
		if (-1 == subsys) {
			CMDQ_ERR("REC: Unsupported memory address 0x%08x\n",
				argA);
			return -EFAULT;
		}
		*pCommand++ = argB;
		*pCommand++ = (CMDQ_CODE_POLL << 24) |
			(argA & 0x0FFFF) |
			((subsys & 0x01F) << subsysBit);
		break;
	case CMDQ_CODE_JUMP:
...
int32_t cmdqRecSetSecure(struct cmdqRecStruct *handle, const bool isSecure)
{
	if (!handle)
		return -EFAULT;
 ...
 *
 */
int32_t cmdqBackupAllocateSlot(cmdqBackupSlotHandle *phBackupSlot,
	uint32_t slotCount)
{
#ifdef CMDQ_GPR_SUPPORT
 ...
 *
 */
int32_t cmdqBackupReadSlot(cmdqBackupSlotHandle hBackupSlot,
	uint32_t slotIndex,
	uint32_t *value)
{
#ifdef CMDQ_GPR_SUPPORT
	...
	*value = cmdqCoreReadWriteAddress(
...
int32_t cmdqBackupWriteSlot(cmdqBackupSlotHandle hBackupSlot,
	uint32_t slotIndex,
	uint32_t value)
{
#ifdef CMDQ_GPR_SUPPORT
 ...
 *  AFTER cmdqRecFlush() returns, or INSIDE the
 * callback of cmdqRecFlushAsyncCallback().
 *
 */
...
int32_t cmdqRecEnablePrefetch(struct cmdqRecStruct *handle)
{
		...
		/* disable pre-fetch marker feature but
		 * use auto prefetch mechanism
		 */
...
int32_t cmdq_rec_finalize_command(struct cmdqRecStruct *handle, bool loop)
{
	int32_t status = 0;
	uint32_t argB = 0;
	...
	if (!handle->finalized) {
		if ((handle->prefetchCount > 0) &&
			cmdq_core_should_enable_prefetch(handle->scenario)) {
			CMDQ_ERR("enable_prefetch:%d, prefetchCount:%d\n",
				cmdq_core_should_enable_prefetch(
					handle->scenario),
				handle->prefetchCount);
	...
	struct cmdqRecStruct *handle)
{
	...
	pDesc->secData.addrMetadataCount =
...
int32_t cmdqRecFlushAsync(struct cmdqRecStruct *handle)
{
	...
	CMDQ_MSG("[Auto Release] Submit ASYNC task scenario: %d, ",
		handle->scenario);
	CMDQ_MSG("priority: %d, engine: 0x%llx, buffer: 0x%p, size: %d\n",
		handle->priority,
		handle->engineFlag,
		handle->pBuffer,
		handle->blockSize);
...
int32_t cmdqRecStartLoop(struct cmdqRecStruct *handle)
{
	...
	CMDQ_MSG("Submit task loop: scenario: %d, priority: %d, ",
	     handle->scenario,
	     handle->priority);
	CMDQ_MSG("engine: 0x%llx, buffer: 0x%p,",
	     handle->engineFlag,
	     handle->pBuffer);
	CMDQ_MSG(" size: %d, callback: 0x%p, data: %d\n",
	    handle->blockSize,
	    &cmdqRecIRQCallback,
	    0);
	...
	desc.pVABase = (cmdqU32Ptr_t) (unsigned long)handle->pBuffer;
	desc.blockSize = handle->blockSize;
	cmdq_rec_setup_sec_data_of_command_desc_by_rec_handle(&desc,
		handle);	/* secure path */
...
int32_t cmdqRecWaitThreadIdleWithTimeout(int threadID, unsigned int retryCount)
{
	...
	if (count > 0 && count < retryCount)
		...
		CMDQ_LOG("check hw thread[%d] idle timeout[%d]\n",
			threadID,
			count);
		CMDQ_LOG("current load thread = %x status = %x\n",
			CMDQ_REG_GET32(CMDQ_CURR_LOADED_THR),
			(CMDQ_REG_GET32(CMDQ_THR_CURR_STATUS(threadID))));
		CMDQ_LOG("CURR_ADDR = 0x%08x END_ADDR = 0x%08x count[%d]\n",
			CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(threadID)),
			CMDQ_REG_GET32(CMDQ_THR_END_ADDR(threadID)),
			pThread->taskCount);
	}
