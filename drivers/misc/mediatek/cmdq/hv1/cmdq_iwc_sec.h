 ...
 * so if following context in cmdq_sec.h,
 * then cmdq_core.h will include cmdq_sec.h
 * but the question is cmdq_sec.h includes cmdq_core.h already.
 * so we extract this context out in a separate file
 * which will not included outside the cmdq folder
 */
