...
static DEVICE_ATTR(error,
			0600,
			cmdqCorePrintError,
			cmdq_driver_dummy_write);
static DEVICE_ATTR(record,
			0600,
			cmdqCorePrintRecord,
			cmdq_driver_dummy_write);
static DEVICE_ATTR(log_level,
			0600,
			cmdqCorePrintLogLevel,
			cmdqCoreWriteLogLevel);
static DEVICE_ATTR(profile_enable,
			0600,
...
static int cmdq_release(struct inode *pInode, struct file *pFile)
{
	...
	if (pNode->mutexFlag) {
		for (index = DISP_MUTEX_MDP_FIRST;
			index < DISP_MUTEX_MDP_FIRST + DISP_MUTEX_MDP_COUNT;
			index++) {
	...
	struct cmdqCommandStruct *pCommand)
{
	...
	if (pCommand->debugRegDump) {
		...
		if (status != 0) {
			CMDQ_ERR("cmdqCoreDebugRegDumpBegin returns %d\n",
				status);
			kernelRegCount = 0;
			kernelRegAddr = NULL;
		}
	...
	if (totalRegCount == 0) {
		...
		regAddrBuf = kcalloc(totalRegCount,
			sizeof(uint32_t),
			GFP_KERNEL);
		if (!regAddrBuf)
			return -ENOMEM;
	...
	struct cmdqReadAddressStruct *req_user)
{
	...
	do {
		...
		/* copy value to user */
		if (copy_to_user(CMDQ_U32_PTR(req_user->values),
				values,
				req_user->count * sizeof(uint32_t))) {
	...
	struct cmdqCommandStruct *pCommand)
{
	...
	if (!pAddrMetadatas) {
		CMDQ_ERR("kzalloc failed, count:%d, alloacted_size:%d\n",
				 pCommand->secData.addrMetadataCount,
				 length);
		return -ENOMEM;
	}
	...
	if (copy_from_user(pAddrMetadatas,
		 ...
		 * addrMetadatas is valid kernel
		 * space buffer address when free it
		 */
		pCommand->secData.addrMetadatas =
		...
		struct cmdqCommandStruct *pCommand)
{
	...
	pCommand->regValue.regValues = (cmdqU32Ptr_t) (unsigned long)
		kzalloc(pCommand->regRequest.count * sizeof(uint32_t),
			GFP_KERNEL);
	pCommand->regValue.count = pCommand->regRequest.count;
	if (!CMDQ_U32_PTR(pCommand->regValue.regValues)) {
	...
	if (pCommand->scenario == CMDQ_SCENARIO_USER_MDP)
		CMDQ_MSG("srcHandle=0x%08x, dstHandle=0x%08x\n",
			pCommand->secData.srcHandle,
			pCommand->secData.dstHandle);
	...
	unsigned long param)
{
	...
	case CMDQ_IOCTL_UNLOCK_MUTEX:
		...
		if (-1 != mutex) {
			pNode = (struct cmdqFileNodeStruct *)
				pFile->private_data;
			spin_lock_irqsave(&pNode->nodeLock, flags);
			pNode->mutexFlag &= ~(1 << mutex);
			spin_unlock_irqrestore(&pNode->nodeLock, flags);
	...
	case CMDQ_IOCTL_EXEC_COMMAND:
		/* CMDQ_LOG("CMDQ_IOCTL_EXEC_COMMAND\n"); */
	...
	case CMDQ_IOCTL_QUERY_USAGE:
		...
		if (copy_to_user((void *)param,
			...
			CMDQ_ERR("QUERY_USAGE copy failed\n");
			return -EFAULT;
		}
		break;
	case CMDQ_IOCTL_ASYNC_JOB_EXEC:
		/* CMDQ_LOG("CMDQ_IOCTL_ASYNC_JOB_EXEC \n");*/
		...
		if (cmdqCoreIsEarlySuspended() &&
			(job.command.scenario == CMDQ_SCENARIO_USER_MDP)) {
			CMDQ_ERR("ASYNC_JOB_EXEC suspended, return\n");
			return -EFAULT;
		}

		/* not support secure path for async ioctl yet */
		if (job.command.secData.isSecure) {
			CMDQ_ERR("not support secure path\n");
			return -EFAULT;
		}
		...
		/* scenario id fixup */
		if ((job.command.scenario == CMDQ_SCENARIO_USER_DISP_COLOR)
			|| (job.command.scenario == CMDQ_SCENARIO_USER_MDP)) {
			CMDQ_VERBOSE("user space request, scenario:%d\n",
		...
		if (status >= 0) {
			...
			if (copy_to_user((void *)param,
					(void *)&job,
					sizeof(struct cmdqJobStruct))) {
				CMDQ_ERR("ASYNC_JOB_EXEC copyfailed\n");
	...
	case CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE:
		/* CMDQ_LOG("CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE\n");*/
		if (copy_from_user(&jobResult,
				(void *)param,
				sizeof(jobResult))) {
		...
		if (cmdqCoreIsEarlySuspended() &&
			(job.command.scenario == CMDQ_SCENARIO_USER_MDP)) {
			CMDQ_ERR("ASYNC_JOB_WAIT_AND_CLOSE suspended\n");
			return -EFAULT;
		}

		/* verify job handle */
		if (!cmdqIsValidTaskPtr((struct TaskStruct *)
		...
		jobResult.regValue.regValues =
			(cmdqU32Ptr_t) (unsigned long)
			(kzalloc(pTask->regCount * sizeof(uint32_t),
			GFP_KERNEL));
		jobResult.regValue.count = pTask->regCount;
		if (!CMDQ_U32_PTR(jobResult.regValue.regValues)) {
	...
	case CMDQ_IOCTL_ALLOC_WRITE_ADDRESS:
		...
		CMDQ_LOG("ALLOC_WRITE_ADDRESS get 0x%08x\n",
			addrReq.startPA);
	...
	case CMDQ_IOCTL_FREE_WRITE_ADDRESS:
		...
		if (copy_from_user(&freeReq,
				(void *)param,
				sizeof(freeReq))) {
			CMDQ_ERR("FREE_WRITE_ADDRESS copyfailed\n");
			return -EFAULT;
		}
	...
	case CMDQ_IOCTL_READ_ADDRESS_VALUE:
		...
		if (copy_from_user(&readReq,
				(void *)param,
				sizeof(readReq))) {
			CMDQ_ERR("READ_ADDRESS_VALUE copy failed\n");
			return -EFAULT;
		}
	...
	case CMDQ_IOCTL_SYNC_BUF_HDCP_VERSION:
		...
		do {
			...
			if (copy_from_user(&syncHandle,
					(void *)param,
					sizeof(syncHandle))) {
				CMDQ_ERR("SYNC_BUF_HDCP copy failed\n");
				return -EFAULT;
			}
			status = cmdq_sec_sync_handle_hdcp_unlock(syncHandle);
			if (status)
				CMDQ_ERR("sync hdcp handle returns %d\n",
					status);
		...
		break;
	default:
		CMDQ_LOG("size of  cmdqCommandStruct is %08zx\n",
			sizeof(struct cmdqCommandStruct));
		CMDQ_LOG("CMDQ_IOCTL_QUERY_CAP_BITS:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_QUERY_CAP_BITS);
		CMDQ_LOG("CMDQ_IOCTL_LOCK_MUTEX:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_LOCK_MUTEX);
		CMDQ_LOG(" CMDQ_IOCTL_UNLOCK_MUTEX:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_UNLOCK_MUTEX);
		CMDQ_LOG(" CMDQ_IOCTL_EXEC_COMMAND:0x%08x\t the error code\n",
			(unsigned int)CMDQ_IOCTL_EXEC_COMMAND);
		CMDQ_LOG(" CMDQ_IOCTL_QUERY_USAGE:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_QUERY_USAGE);
		CMDQ_LOG(" CMDQ_IOCTL_ASYNC_JOB_EXEC:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_ASYNC_JOB_EXEC);
		CMDQ_LOG(" CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_ASYNC_JOB_WAIT_AND_CLOSE);
		CMDQ_LOG(" CMDQ_IOCTL_ALLOC_WRITE_ADDRESS:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_ALLOC_WRITE_ADDRESS);
		CMDQ_LOG(" CMDQ_IOCTL_FREE_WRITE_ADDRESS:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_FREE_WRITE_ADDRESS);
		CMDQ_LOG(" CMDQ_IOCTL_READ_ADDRESS_VALUE:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_READ_ADDRESS_VALUE);
		CMDQ_LOG(" CMDQ_IOCTL_QUERY_CAP_BITS:0x%08x\n",
			(unsigned int)CMDQ_IOCTL_QUERY_CAP_BITS);
		CMDQ_ERR("possible reason: kernel headers file not sync\n");
...
#ifdef CONFIG_COMPAT
static long cmdq_ioctl_compat(struct file *pFile,
	unsigned int code,
	unsigned long param)
{
	switch (code) {
	...
	do {
		if (cmdq_dev_get_irq_id() == IRQ) {
			...
			for (index = 0;
				...
				/* so we mark irqStatus to 1
				 * to denote finished processing
	...
	do {
		if (cmdq_dev_get_irq_id() == IRQ) {
			...
			for (index = 0;
				...
				/* so we mark irqStatus to 1
				 * to denote finished processing
				 * and we can early-exit if no
				 * more threads being asserted
				 */
				irqStatus |= (1 << index);
...
static int cmdq_create_debug_entries(void)
{
	...
	if (debugDirEntry) {
		...
		entry = proc_create("status",
					0440,
					debugDirEntry,
					&cmdqDebugStatusOp);
		entry = proc_create("error",
					0440,
					debugDirEntry,
					&cmdqDebugErrorOp);
		entry = proc_create("record",
					0440,
					debugDirEntry,
					&cmdqDebugRecordOp);
		entry = proc_create("log_level",
					0660,
					debugDirEntry,
					&cmdqDebugLevelOp);
	}
...
static int cmdq_probe(struct platform_device *pDevice)
{
	...
	status = alloc_chrdev_region(&gCmdqDevNo,
				0,
				1,
	...
	object = device_create(gCMDQClass,
		NULL,
		gCmdqDevNo,
		NULL,
		CMDQ_DRIVER_DEVICE_NAME);
	...
	status =
		request_irq(cmdq_dev_get_irq_id(),
			cmdq_irq_handler,
			IRQF_TRIGGER_LOW,
			CMDQ_DRIVER_DEVICE_NAME,
			gCmdqCDev);
	 ...
	 * MUST registet secure IRQ to GIC in normal world
	 * to ensure it will be initialize correctly
	...
	status =
		request_irq(cmdq_dev_get_irq_secure_id(),
			cmdq_sec_irq_handler,
			IRQF_TRIGGER_LOW,
			"TEE IRQ",
			gCmdqCDev);
	if (status != 0) {
