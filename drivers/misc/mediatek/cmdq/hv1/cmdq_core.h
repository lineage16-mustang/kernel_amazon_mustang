 ...
 */
/*  */
/* when 33 bit enabled(4GB ram),
 * 0x_0_xxxx_xxxx and 0x_1_xxxx_xxxx access is same for CPU
 */
/*  */
/*  */
/* 0x0            0x0_4000_0000        0x1_0000_0000        0x1_4000_0000 */
/* |---1GB HW addr---|------3GB DRAM------|
 * ----1GB DRAM(new)----|-----3GB DRAM(same)------|
 */
/* |                                               | */
/* |<--------------4GB RAM support---------------->| */
/*  */
...
typedef int32_t(*CmdqDebugRegDumpBeginCB) (uint32_t taskID,
	uint32_t *regCount,
	uint32_t **regAddress);
typedef int32_t(*CmdqDebugRegDumpEndCB) (uint32_t taskID,
	uint32_t regCount,
	uint32_t *regValues);
...
struct TaskStruct {
	...
	uint32_t bufferSize;
	/* is the command buffer emergency buffer? */
	...
	uint32_t *pCMDEnd;
	int32_t reorder;
	/* ASYNC: CMDQ_INVALID_THREAD if not running */
	int32_t thread;
	...
	/* store GPT counter when it starts and ends */
...
struct ThreadStruct {
	 ...
	 * .note it is align task attachment,
	 * so use cmdqExecLock to ensure atomic access
	 */
	uint32_t allowDispatching;
};

struct RecordStruct {
	/* calling SW thread tid */
	pid_t user;
	/* task scenario */
	int32_t scenario;
	/* task priority (not thread priority) */
	int32_t priority;
	/* allocated thread */
	int32_t thread;
	int32_t reorder;
	...
	/* true for secure task */
	bool isSecure;
	/* epoch time of IOCTL/Kernel API call */
	CMDQ_TIME submit;
	/* epoch time of enable HW thread */
	CMDQ_TIME trigger;
	/* epoch time of start waiting for task completion */
	CMDQ_TIME beginWait;
	/* epoch time of IRQ event */
	CMDQ_TIME gotIRQ;
	/* epoch time of SW thread leaving wait state */
	CMDQ_TIME wakedUp;
	/* epoch time of task finish */
	CMDQ_TIME done;
 ...
 *                 since this disables thread completion handling,
 *			do not set this CB
...
/**
 * Set a given value to CMDQ secure HW thread's
 * THR executed count to secShareMemory
 * Parameter:
 *    thread: [IN] CMDQ thread id
 *    cookie: [IN] the desired value,
 * which should copy from CMDQ_THR_EXEC_CMDS_CNT of secure thread
 *  Return:
 ...
 */
	...
	int32_t cmdq_core_parse_instruction(const uint32_t *pCmd,
		char *textBuf,
		int bufLen);
	void cmdq_core_dump_instructions(uint64_t *pInstrBuffer,
		uint32_t bufferSize);
...
char *cmdq_core_get_clk_name(enum CMDQ_CLK_ENUM clk_enum);
...
void cmdq_core_dump_instructions_to_file(uint64_t *pInstrBuffer,
	uint32_t bufferSize,
	const char *fileName);
