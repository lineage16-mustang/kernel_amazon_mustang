 ...
 * HACK: use m4u_larb0_enable() to lab0 on/off
 * to ensure larb0 restore and clock on/off sequence
...
static struct EmergencyBufferStruct gCmdqEmergencyBuffer[
	CMDQ_EMERGENCY_BLOCK_COUNT];
...
static void cmdqCoreHandleError(int32_t thread,
	int32_t value,
	CMDQ_TIME *pGotIRQ);
static void cmdqCoreHandleDone(int32_t thread,
	int32_t value,
	CMDQ_TIME *pGotIRQ);
...
struct task_struct *cmdq_print_log_task;
...
static int cmdq_core_print_log_kthread(void *data)
{
	...
	while (1) {
		...
		cmdqCoreLongString(needPrintLog,
			longMsg,
			&msgOffset,
			&msgMAXSize,
			"gTaskCount print,");
	...
	if (pPrint->enablePrint) {
		if (!cmdq_print_log_task) {
			...
			if (IS_ERR(cmdq_print_log_task))
				CMDQ_ERR("can not create kthread\n");
	...
	const gfp_t flag)
{
	...
	do {
		...
		(minForceEmergencyBufferSize < size)) {
			CMDQ_LOG("%s failed force use emergency buffer\n",
					__func__);
			break;
		}
#endif
...
static bool cmdq_core_uninit_emergency_buffer(void)
{
	...
	for (i = 0; i < CMDQ_EMERGENCY_BLOCK_COUNT; ++i) {
		...
		cmdq_core_free_hw_buffer(cmdq_dev_get(),
			...
			buf->pa);
		if (buf->used) {
			CMDQ_ERR("buffer %d, 0x%p, 0x%pa still using\n",
					i, buf->va, &buf->pa);
		}
	}
...
int32_t cmdq_core_set_secure_IRQ_status(uint32_t value)
{
	...
	return 0;
#else
	CMDQ_ERR("func:%s failed since SVP not support in this proj\n",
		__func__);
	return -EFAULT;
#endif
}
	...
	char *buf)
{
	...
	const enum CMDQ_ENG_ENUM engines[] = {
		CMDQ_FOREACH_STATUS_MODULE_PRINT(GENERATE_ENUM) };
	...
	for (listIdx = 0; listIdx < ARRAY_SIZE(lists); listIdx++) {
		...
		list_for_each(p, lists[listIdx]) {
			...
			pBuffer +=
			sprintf(pBuffer,
					"State:%d, VABase:0x%p, MVABase:%x\n",
					pTask->taskState,
					pTask->pVABase,
					(uint32_t)pTask->MVABase);
			pBuffer +=
			sprintf(pBuffer,
				"Size:%d, Scenario:%d, Priority:%d\n",
				pTask->commandSize,
				pTask->scenario,
				pTask->priority);
			...
			pBuffer +=
			sprintf(pBuffer,
				"Flag:0x%08llx,VAEnd:0x%p,Tri:%lld\n",
				pTask->engineFlag,
				pTask->pCMDEnd,
				pTask->trigger);
			pBuffer +=
			sprintf(pBuffer,
				"IRQ:%lld, Wait:%lld, Wake Up:%lld\n",
				pTask->gotIRQ,
				pTask->beginWait,
				pTask->wakedUp);
			++index;
	...
	for (index = 0; index < CMDQ_MAX_THREAD_COUNT; index++) {
		...
		for (inner = 0; inner < CMDQ_MAX_TASK_IN_THREAD; inner++) {
			...
			pBuffer +=
			sprintf(pBuffer,
					"Size:%d,Last Command:0x%08x:0x%08x\n",
					pTask->commandSize,
					pTask->pCMDEnd[-1],
					pTask->pCMDEnd[0]);
	...
	pBuffer +=
	sprintf(pBuffer,
			"Index: %d, Enabled: %d, IRQ: 0x%08x, Thread PC:0x%08x,",
			thread, value[8], value[4], value[0]);

	pBuffer +=
	sprintf(pBuffer,
			" End: 0x%08x, Wait Token: 0x%08x\n",
			value[1], value[2]);

	pBuffer +=
	sprintf(pBuffer,
			"Curr Cookie: %d, Wait Cookie: %d, Next Cookie: %d, ",
			value[3], pThread->waitCookie, pThread->nextCookie);
	...
	pBuffer +=
	sprintf(pBuffer,
			"Timeout Cycle:%d, Status:0x%08x, IRQ_EN:0x%08x\n",
		value[5],
		value[6],
		value[7]);
	...
	length = pBuffer - buf;
	if (length < PAGE_SIZE)
		pBuffer += primary_display_check_path(pBuffer,
			(PAGE_SIZE - length));
#endif
	...
	int bufLen)
{
	...
	unit[3] = "ms";
	unit[4] = "ms";
	CMDQ_GET_TIME_IN_MS(pRecord->submit,
		pRecord->done,
		totalTime);
	CMDQ_GET_TIME_IN_MS(pRecord->submit,
		pRecord->trigger,
	...
	CMDQ_GET_TIME_IN_MS(pRecord->submit,
		pRecord->beginWait,
		beginWaitTime)
	CMDQ_GET_TIME_IN_MS(pRecord->trigger,
		pRecord->gotIRQ,
		IRQTime)
	CMDQ_GET_TIME_IN_MS(pRecord->trigger,
		pRecord->wakedUp,
	...
	length += snprintf(buf + length,
		bufLen - length,
		"%4d:(%5d,%2d,0x%012llx,%2d,%d)(%02d,%02d)(%5dns:%lld,%lld)",
 ...
 *	return:	return negative means error. return 0
 * or postive to represent the bytes written to file
 */
	...
	char __user *pPage, size_t Count, loff_t *ppos)
{
	...
	return simple_read_from_buffer(pPage,
		Count, ppos, buffer, strlen(buffer));
#else
	static int num;
	int length = 0;
	int notSend = 0;
	...
	if (*ppos >= length || Count == 0) {
		CMDQ_ERR("pos:%d, length:%d, Count:%d",
			(int)(*ppos),
			length,
			(int)Count);
		return 0;
...
int cmdqCorePrintStatusSeq(struct seq_file *m, void *v)
{
	...
	do {
		...
		/* this returns (1 + index of least bit set)
		 * or 0 if input is 0.
		 */
		coreExecThread = __builtin_ffs(value[0]) - 1;
		seq_printf(m,
			"IRQ flag:0x%08x, Execing:%d, Exec Thread:%d, ",
			CMDQ_REG_GET32(CMDQ_CURR_IRQ_STATUS),
			(0x80000000 & value[0]) ? 1 : 0,
			coreExecThread);
		seq_printf(m,
			"CMDQ_CURR_LOADED_THR: 0x%08x\n",
			value[0]);
		if (coreExecThread != CMDQ_INVALID_THREAD) {
	...
	for (index = 0; index < CMDQ_MAX_THREAD_COUNT; index++) {
		...
		if (pThread->taskCount > 0) {
			...
			for (inner = 0;
				...
				seq_printf(m,
					"Scn:%d,VABase:0x%p,MVABase:%pa, ",
					pTask->scenario,
					pTask->pVABase,
					&pTask->MVABase);

				seq_printf(m,
					"Size:%d,Last CMD: 0x%08x:0x%08x\n",
					pTask->commandSize,
					pTask->pCMDEnd[-1],
					pTask->pCMDEnd[0]);
...
void cmdq_task_free_task_command_buffer(struct TaskStruct *pTask)
{
	if (pTask->pVABase) {
		if (pTask->useEmergencyBuf) {
			cmdq_core_free_emergency_buffer(pTask->pVABase,
				pTask->MVABase);
		} else {
			cmdq_core_free_hw_buffer(cmdq_dev_get(),
				pTask->bufferSize,
...
static int32_t cmdq_core_task_realloc_buffer_size(struct TaskStruct *pTask,
	uint32_t size)
{
	...
	do {
		...
		if (size <= CMDQ_EMERGENCY_BLOCK_SIZE)
			cmdq_core_alloc_emergency_buffer(&pNewBuffer,
				&newMVABase);
	...
	/* we should keep track of pCMDEnd and cmdSize
...
static struct TaskStruct *cmdq_core_task_create(void)
{
	...
	pTask = (struct TaskStruct *)kmem_cache_alloc(
		gCmdqContext.taskCache, GFP_KERNEL);
...
#define DECLARE_CMDQ_EVENT(name, val) \
	CMDQ_REG_SET32(CMDQ_SYNC_TOKEN_UPD, (CMDQ_SYNC_TOKEN_MAX & name));
#include "cmdq_event.h"
#undef DECLARE_CMDQ_EVENT
...
int32_t cmdqCoreInitialize(void)
{
	...
	WARN_ON(atomic_read(&gCmdqThreadUsage));
	WARN_ON(atomic_read(&gSMIThreadUsage));
	...
	gCmdqPrint.CmdqPrintWQ = create_singlethread_workqueue(
		"cmdq_print_count_of_submitted_task");
...
static void cmdq_core_append_command_into_task(struct TaskStruct *pTask,
	uint32_t argA,
	uint32_t argB)
{
	...
	int32_t prevID)
{
	...
	for (loop = 1; loop < (CMDQ_MAX_TASK_IN_THREAD - 1); loop++, nextID++) {
		...
		for (searchLoop = (loop + 1);
			searchLoop < CMDQ_MAX_TASK_IN_THREAD;
			searchLoop++, searchID++) {
...
#define CMDQ_PROFILE_COMMAND
	...
	/* HACK: let blockSize >= commandSize + 1 * CMDQ_INST_SIZE
	 * to prevnet GCE stop wrongly
	 ...
	 *         = begin of task_2, so...GCE stop at begin of
	 *			task_2's command and never exec task_2 & task_3
	 */
	...
	if (status < 0) {
		CMDQ_ERR("failed to realloc, pTask=0x%p, requireSize=%d\n",
			pTask,
			pTask->commandSize +
				prependBufferSize + extraBufferSize);
		return status;
	}

	/* pCMDEnd start from beginning.
	 * Note it is out-of-sync with pTask->commandSize.
	 */
	...
	if (profileCommand) {
...
#ifdef CMDQ_PROFILE_COMMAND_TRIGGER_LOOP
			/* HACK: insert profile instr after
			 * WAIT TE_EOF (command mode only)
			 * note content of trigger loop
		...
		pTask->pCMDEnd =
		pTask->pVABase +
			(copiedBeforeProfile / sizeof(pTask->pVABase[0])) - 1;
	...
	pTask->pCMDEnd = pTask->pVABase +
		(pTask->commandSize / sizeof(pTask->pVABase[0])) - 1;
	...
	if (!pTask->regCount && !extraBufferSize)
		return 0;

	/* move EOC+JUMP to the new end */
	memcpy(pTask->pCMDEnd + 1 - 4 +
		(extraBufferSize / sizeof(pTask->pCMDEnd[0])),
		&pTask->pCMDEnd[-3],
		2 * CMDQ_INST_SIZE);
	...
	if (pTask->regCount) {
		...
		cmdq_core_get_reg_id_from_hwflag(pTask->engineFlag,
			&valueRegId,
			&destRegId,
			&regAccessToken);

		/* use SYNC TOKEN to make sure only
		 * 1 thread access at a time
		 * bit 0-11: wait_value
		 * bit 15: to_wait, true
		...
		for (i = 0; i < pTask->regCount; ++i) {
			cmdq_core_insert_backup_instruction(pTask,
				CMDQ_U32_PTR(pCommandDesc->regRequest.
					regAddresses)[i],
				pTask->regResultsMVA +
	...
	if (pTask && pTask->loopCallback) {
		if (((pTask->pCMDEnd[0] & 0xFF000000) >> 24) ==
				CMDQ_CODE_JUMP) {
	...
	unsigned long loopData)
{
	...
	CMDQ_MSG("-->TASK: acquire task : 0x%p, size: %d, Eng: 0x%016llx\n",
			CMDQ_U32_PTR(pCommandDesc->pVABase),
			pCommandDesc->blockSize,
	...
	do {
		...
		pTask->privateData =  (cmdqU32Ptr_t)(unsigned long)
	...
	enum CMDQ_SCENARIO_ENUM scenario)
{
	...
	if (enable) {
		...
		if (!atomic_read(&gCmdqThreadUsage)) {
			/* CMDQ init flow: */
			/* 1. clock-on */
			/* 2. reset all events */
		...
		if (!cmdq_core_is_disp_scenario(scenario) &&
			(scenario != CMDQ_SCENARIO_TRIGGER_LOOP)) {
			if (!atomic_read(&gSMIThreadUsage)) {
				CMDQ_VERBOSE("[CLOCK] SMI clock enable %d\n",
					scenario);
				cmdq_core_enable_common_clock_locked_impl(
					enable);
			}
		...
		CMDQ_VERBOSE("[CLK] Disable CMDQ(GCE) Clock test=%d SMI %d\n",
			atomic_read(&gCmdqThreadUsage),
			atomic_read(&gSMIThreadUsage));
	...
	enum CMDQ_SCENARIO_ENUM scenario)
{
	...
	if (cmdq_core_is_group_flag(CMDQ_GROUP_ISP, engineFlag)) {
		...
		if (!pCallback[CMDQ_GROUP_ISP].clockOn) {
			...
			status =
			pCallback[CMDQ_GROUP_ISP].clockOn(
			...
			if (status < 0)
				CMDQ_ERR("CLK: enable group %d clock failed\n",
					CMDQ_GROUP_ISP);
	...
	for (index = CMDQ_MAX_GROUP_COUNT - 1; index >= 0; --index) {
		...
		if (cmdq_core_is_group_flag((enum CMDQ_GROUP_ENUM)index,
			...
			if (!pCallback[index].clockOn) {
				CMDQ_ERR("CLK: enable group %d clockOn NULL\n",
					index);
				continue;
			}
...
/* traverse gCmdqContext.taskWaitList and find the first task
 * which has engine conflict with current task
 * if pFirstWaitTask is null, then this task can acquire HW thread.
 * if pFirstWaitTask is not null, then check whether pFirstWaitTask's
 * secure flag is same as current task's flag
 * true for can
 * false for can not
 */
static const int32_t cmdq_core_can_start_to_acquire_HW_thread_unlocked(
	const uint64_t
	engineFlag,
	const bool isSecure)
{
	...
	/* find the first waiting task with
	 * OVERLAPPED engine flag with pTask
	 */
	list_for_each(p, &gCmdqContext.taskWaitList) {
	...
	if (preferSecurePath == isSecure) {
		...
		cmdqCoreLongString(false, longMsg, &msgOffset, &msgMAXSize,
			"THR:is sec(%d, eng:0x%llx) as first waiting task",
			isSecure, engineFlag);
	...
	CMDQ_VERBOSE("is not the first waiting task(0x%p), yield.\n",
				pFirstWaitingTask);
	status = -EFAULT;
...
static bool cmdq_core_check_engine_conflict_for_thread_dispatch_locked(
	const uint64_t
	engineFlag,
	bool forceLog,
	const bool isSecure,
	...
	for (index = 0;
		...
		if (pEngine[index].currOwner == CMDQ_INVALID_THREAD) {
			...
			if (forceLog) {
				...
				msgOffset = 0;
				msgMAXSize = CMDQ_LONGSTRING_MAX;
				cmdqCoreLongString(true,
					longMsg,
					&msgOffset,
					&msgMAXSize,
					"THR: try locate on thr %d but eng %d",
							thread, index);
				cmdqCoreLongString(true,
					longMsg,
					&msgOffset,
					&msgMAXSize,
					" also occupied by thr %d, sec:%d\n",
	...
	const bool isSecure)
{
	...
	CMDQ_VERBOSE("find a free thr, eng:0x%llx, scenario: %d, sec:%d\n",
	...
	thread =
	cmdq_core_get_thread_index_from_scenario_and_secure_data(
		scenario,
		isSecure);
	...
	if (false == cmdq_core_is_disp_scenario(scenario)) {
		isEngineConflict =
		cmdq_core_check_engine_conflict_for_thread_dispatch_locked(
			engineFlag,
			forceLog,
			isSecure,
			&thread);
	...
	if ((!isEngineConflict) && (thread == CMDQ_INVALID_THREAD)) {
		 ...
		 */
		const bool isDisplayThread = (thread_prio >
			CMDQ_THR_PRIO_DISPLAY_TRIGGER);
		int startIndex = isDisplayThread ?
			0 :
		...
		for (index = startIndex; index < endIndex; ++index) {
			...
			if ((pThread[index].taskCount == 0)
			&& (pThread[index].allowDispatching == 1)) {

				CMDQ_VERBOSE
					("thr %d, taskCount:%d, allow:%d\n",
	...
	if (pThread[thread].taskCount >= CMDQ_MAX_TASK_IN_THREAD) {
		if (!bCountEnable)
			cmdq_core_enable_print_log(true);
		if (forceLog) {
	...
	if (pThread[thread].
	pCurTask[pThread[thread].nextCookie % CMDQ_MAX_TASK_IN_THREAD]
		!= NULL) {
		if (!bCountEnable)
			cmdq_core_enable_print_log(true);
		if (forceLog) {
			if (thread >= 0 && thread < CMDQ_MAX_THREAD_COUNT) {
				...
				gNextCookieOccupied[thread] =
					pThread[thread].nextCookie;
				CMDQ_MSG("thread %d nextCookie %d already has task\n",
					 thread, pThread[thread].nextCookie);
			}
		} else {
			CMDQ_VERBOSE("thr %d nextCookie %d already has task\n",
				thread,
				pThread[thread].nextCookie);
		}
	...
	enum CMDQ_SCENARIO_ENUM scenario)
{
	...
	CMDQ_VERBOSE("CLK: Disable clk 0x%llx, enginesNotUsed 0x%llx\n",
	...
	if (cmdq_core_is_group_flag(CMDQ_GROUP_ISP, engineFlag)) {
		...
		if (!pCallback[CMDQ_GROUP_ISP].clockOff) {
			...
			if (status < 0) {
				CMDQ_ERR("CLOCK: disable %d clockOff failed\n",
						CMDQ_GROUP_ISP);
			}
		}
	}
	...
	for (index = 0; index < CMDQ_MAX_GROUP_COUNT; ++index) {
		...
		if (cmdq_core_is_group_flag((enum CMDQ_GROUP_ENUM)index,
				enginesNotUsed)) {
			CMDQ_MSG("CLK: Disable eng %d flag=0x%llx clockOff\n",
				index,
				enginesNotUsed);
			if (!pCallback[index].clockOff) {
				CMDQ_ERR("CLK:Disable eng %d clk func NULL\n",
					index);
				continue;
			}
			status = pCallback[index].clockOff(
				gCmdqEngineGroupBits[index] &
				enginesNotUsed);
			if (status < 0)
				CMDQ_ERR("CLK: Disable engine %d clk failed\n",
					index);
...
static void cmdq_core_reset_hw_engine(int32_t engineFlag)
{
	...
	for (index = 0; index < CMDQ_MAX_GROUP_COUNT; ++index) {
		if (cmdq_core_is_group_flag((enum CMDQ_GROUP_ENUM)index,
			...
			if (status < 0)
				CMDQ_ERR("Reset engine group %d clock fail\n",
					index);
...
int32_t cmdq_core_suspend_HW_thread(int32_t thread)
{
	 ...
	 * if already disabled, treat as
	 * suspend successful but print error log
	 */
	enabled = CMDQ_REG_GET32(CMDQ_THR_ENABLE_TASK(thread));
	if (0 == (0x01 & enabled)) {
		CMDQ_LOG("[WARNING] thread %d suspend not effect, enable%d\n",
			thread,
			enabled);
		return 0;
	}
	...
	uint32_t insts[4])
{
	...
	if ((!pTask) || (!pTask->pVABase) || (thread == CMDQ_INVALID_THREAD)) {
		CMDQ_ERR("invalid param, pTask %p,VABase:%p, thread:%d\n",
	...
	if (((uint8_t *) pTask->pVABase <= pInst) &&
		(pInst <= (uint8_t *) pTask->pCMDEnd)) {
...
#undef DECLARE_CMDQ_SUBSYS
#define DECLARE_CMDQ_SUBSYS(addr, id, grp, base) \
	case id: base_addr = addr; break;
	...
	const char *tag)
{
	...
	if (pcVA) {
		...
		if (true == pTask->secData.isSecure) {
			CMDQ_LOG("[%s]Thread %d PC(VA): HIDDEN for secure\n",
					tag, thread);
		} else {
...
static void cmdq_core_dump_status(const char *tag)
{
	...
	CMDQ_LOG("[%s]CMDQ_CURR_LOADED_THR: 0x%08x\n",
			tag,
			value[0]);
...
static void cmdq_core_dump_disp_trigger_loop(void)
{
	...
	CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT].loopCallback) {
		...
		cmdq_core_dump_pc(gCmdqContext.thread[
			CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT].pCurTask[1],
			CMDQ_MAX_HIGH_PRIORITY_THREAD_COUNT,
			"INFO");
		...
		CMDQ_LOG("CMDQ_SYNC_TOKEN_VAL of %s is %d\n",
			cmdq_core_get_event_name(CMDQ_EVENT_DISP_RDMA0_EOF),
			regValue);
	}
}
...
static void cmdq_core_dump_thread_pc(const int32_t thread)
{
	...
	for (i = 0; i < CMDQ_MAX_TASK_IN_THREAD; i++) {
		...
		cmdq_core_parse_instruction(pcVA, parsedInstruction,
					sizeof(parsedInstruction));
		CMDQ_LOG("[INFO]task:%p(index:%d), Thread %d PC(VA): 0x%p\n",
			pTask,
			i,
			thread,
			pcVA);

		CMDQ_LOG("[INFO]0x%08x:0x%08x => %s\n",
			insts[2],
			insts[3],
			parsedInstruction);
...
{
	CMDQ_ERR("Task: 0x%p, Scenario: %d, State: %d, Priority: %d\n",
		pTask,
		pTask->scenario,
		pTask->taskState,
		pTask->priority);
	CMDQ_ERR("Flag: 0x%016llx, VABase: 0x%p\n",
		pTask->engineFlag,
		pTask->pVABase);
	...
	CMDQ_ERR("Reord:%d,Trigger:%lld,got_IRQ:%lld,Wait:%lld,Finish:%lld\n",
			pTask->reorder,
			pTask->trigger,
			pTask->gotIRQ,
			pTask->beginWait,
...
bool cmdq_core_verfiy_command_desc_end(struct cmdqCommandStruct *pCommandDesc)
{
	...
	if (!CMDQ_U32_PTR(pCommandDesc->pVABase) ||
		...
		return false;
	if (true == cmdq_core_is_request_from_user_space(
			pCommandDesc->scenario)) {
	...
	if ((pCMDEnd[-3] & 0x1) != 1) {
		CMDQ_ERR
			("0x%p not throw IRQ (%08x:%08x), pEnd:%p(%p, %d)\n",
	...
	((pCMDEnd[0] & 0xFF000000) >> 24) != CMDQ_CODE_JUMP) {
		CMDQ_ERR("0x%p not end in EOC+JUMP(%08x:%08x, %08x:%08x)\n",
			pCommandDesc, pCMDEnd[-3],
			pCMDEnd[-2],
			pCMDEnd[-1],
			pCMDEnd[0]);
...
static void cmdq_core_dump_task_with_engine_flag(uint64_t engineFlag)
{
	...
	list_for_each(p, &gCmdqContext.taskActiveList) {
		...
		if (pDumpTask && (engineFlag & pDumpTask->engineFlag)) {
			CMDQ_ERR("Thr:%d,Task:%p,VABase:%p,MVABase:0x%x, ",
				(pDumpTask->thread),
				(pDumpTask),
				(pDumpTask->pVABase),
				(uint32_t)(pDumpTask->MVABase));

			CMDQ_ERR("Size:%d,Flag:0x%08llx,",
	...
	const bool dumpCmd)
{
	...
	if (dumpCookie) {
		CMDQ_ERR("Curr:%d, Wait:%d, Next:%d, Task Count %d\n",
				cookie,
				pThread->waitCookie,
				pThread->nextCookie,
				pThread->taskCount);
	...
	int bufLen)
{
	...
	case CMDQ_CODE_POLL:
		...
		if (argA & (1 << 23)) {
			...
			const uint32_t addr =
				cmdq_core_subsys_to_reg_addr(argA);

			reqLen = snprintf(textBuf,
					bufLen,
					"addr=0x%08x [%s],use_mask=%d\n",
					(addr & 0xFFFFFFFE),
					cmdq_core_parse_module_from_reg_addr(
						addr),
					(addr & 0x1));
			bufLen -= reqLen;
			textBuf += reqLen;
...
void cmdq_core_dump_instructions(uint64_t *pInstrBuffer, uint32_t bufferSize)
{
	...
	while (pBuffer <=
		(uint64_t *)((uint8_t *)(pInstrBuffer) + bufferSize)) {
		memset(textBuffer, 0, 100);
		cmdq_core_parse_instruction((uint32_t *)pBuffer,
			textBuffer,
			99);
...
void cmdq_core_dump_instructions_to_file(
	uint64_t *pInstrBuffer,
	uint32_t bufferSize,
	const char *fileName)
{
	uint64_t *pBuffer = pInstrBuffer;
...
static void cmdq_core_track_task_record(
	struct TaskStruct *pTask,
	uint32_t thread)
{
...
static void cmdq_core_copy_error_task_instruction(
	struct ErrorTaskItemStruct *pErrorTask,
	const struct TaskStruct *pTask)
{
	int32_t index = 0;
	...
	int32_t thread)
{
	...
	if (false == cmdq_core_is_a_secure_thread(thread)) {
		...
		CMDQ_ERR("Index: %d, Enabled: %d, IRQ: 0x%08x\n",
			thread, value[8], value[4]);
		CMDQ_ERR("Thread PC: 0x%08x, End: 0x%08x, Wait Token:0x%08x\n",
			value[0], value[1], value[2]);
		CMDQ_ERR("Curr Cookie: %d, Wait Cookie: %d\n",
				value[3], pThread->waitCookie);
		CMDQ_ERR("Next Cookie: %d, Task Count %d,\n",
				pThread->nextCookie, pThread->taskCount);
	...
	if (pTask) {
		...
		CMDQ_ERR
			("[CMDQ] Begin of Error Task[%p] trigger[%lld]\n",
			pTask, pTask->trigger);
	...
	for (index = 0; index < CMDQ_MAX_GROUP_COUNT; ++index) {
		if (!cmdq_core_is_group_flag(
			(enum CMDQ_GROUP_ENUM)index, pTask->engineFlag))
			continue;
		...
		pCallback[index].dumpInfo(
			(gCmdqEngineGroupBits[index] & pTask->engineFlag),
			cmdq_core_get_log_level());
	}

	/* force dump DISP for DISP scenario with 0x0 engine flag */
	if ((pTask->engineFlag == 0) &&
		...
		uint32_t *instB)
{
	...
	do {
		...
		if (cmdq_core_get_pc(pTask, thread, insts)) {
			...
			module = cmdq_core_parse_error_module_by_hwflag_impl(
				pTask);
			if (module)
				break;
	...
	newTaskState)
{
	...
	if ((!pThread) || (index >= CMDQ_MAX_TASK_IN_THREAD) || (index < 0)) {
		CMDQ_ERR("remove task err THR[0x%p],slot[%d],State[%d]\n",
	...
	/* note timing to switch a task to
	 * done_status(_ERROR, _KILLED, _DONE)
	 */
	...
	if (pTask->taskState != TASK_STATE_BUSY) {
		CMDQ_ERR("remove task,err[%d].THR[0x%p],slot[%d],status[%d]\n",
		pTask->taskState,
		pThread,
		index,
		newTaskState);
								...
								uint32_t thread)
{
	...
	if (pExecTask && (pExecTask == pTask)) {
		...
		CMDQ_REG_SET32(CMDQ_THR_CURR_ADDR(thread),
			CMDQ_PHYS_TO_AREG(
			pTask->MVABase + pTask->commandSize - 16));
		...
		for (index = (cookie % CMDQ_MAX_TASK_IN_THREAD);
			loop > 0;
			loop--, index++) {
	...
	int32_t thread)
{
	...
	/* pTask->taskState flag is updated in
	 * IRQ handlers like cmdqCoreHandleDone.
	 */
	retryCount = 0;
	...
	while (waitQ == 0 && retryCount < CMDQ_PREDUMP_RETRY_COUNT) {
		CMDQ_LOG
			("timeout Pre-dump(%d) pTask[0x%p], trigger[%lld]\n",
		...
		if (retryCount == 0) {
			...
			cmdq_core_dump_instructions(
				(uint64_t *)(pTask->pVABase),
				pTask->commandSize);
			CMDQ_ERR("----pTask[%p] Trigger[%lld] dump finish--\n",
					pTask,
					pTask->trigger);
			if (gCmdqContext.errorTask.errTaskCount <
				CMDQ_MAX_ERROR_COUNT) {
				/* copy error task to error task buffer */
				pErrorTask =
					&gCmdqContext.errorTask.errorTaskBuffer[
						gCmdqContext.errNum];
...
/* bool markAsErrorTask = false; */
	...
	if (pTask->taskState == TASK_STATE_DONE) {
		...
		/* remove all tasks in tread since we
		 * have reset HW thread in SWd
		 */
		for (i = 0; i < CMDQ_MAX_TASK_IN_THREAD; i++) {
			pTask = pThread->pCurTask[i];
			if (!pTask)
	...
	/* cmdq_core_parse_error(pTask,
	 * thread, &module, &irqFlag, &instA, &instB);
	 */
...
static void cmdq_core_remove_error_task_from_thread_task_queue(
	struct TaskStruct *pTask,
	struct ThreadStruct *pThread,
	long threadPC,
	int32_t cookie,
	int thread)
{
	...
	if (pTask->pCMDEnd && 0x10000001 == pTask->pCMDEnd[0]) {
		pNextTask = cmdq_core_search_task_by_pc(pTask->pCMDEnd[-1],
			pThread);
	} else {
	...
	for (index = 0; index < CMDQ_MAX_TASK_IN_THREAD; ++index) {
		...
		if (pPrevTask && pPrevTask->pCMDEnd &&
			...
			/* Give up fetched command, invoke CMDQ HW
			 * to re-fetch command buffer again.
			 */
			...
			int32_t thread, const int32_t waitQ)
{
	...
	cmdq_core_remove_error_task_from_thread_task_queue(pTask,
		pThread,
		threadPC,
		cookie,
		thread);
exist:
	...
	/* throw AEE */
	op = (instA & 0xFF000000) >> 24;

	switch (op) {
	case CMDQ_CODE_WFE:
		CMDQ_AEE(module,
			"%s IRQ:0x%02x, INST:(0x%08x, 0x%08x), OP:WAIT:%s\n",
			module,
			irqFlag,
			instA,
			instB,
	...
	default:
		CMDQ_AEE(module,
			"%s IRQ:0x%02x, INST:(0x%08x, 0x%08x), OP:%s\n",
			module,
			irqFlag,
			instA,
			instB,
 ...
 *     SW should notify GCE to re-fetch command in order
 *     to ensure inconsistent command buffer content
	...
	long timeout_jiffies)
{
	...
	if (thread == CMDQ_INVALID_THREAD) {
		...
		/* wait for acquire thread (this is done
		 * by cmdq_core_consume_waiting_list);
		 */
		...
		if (0 == waitQ || CMDQ_INVALID_THREAD == pTask->thread) {
			...
			if (pTask->thread == CMDQ_INVALID_THREAD) {
				 ...
				 * or starved to death
				 */
				CMDQ_ERR("task 0x%p timeout invalid thread\n",
					pTask);
				cmdq_core_dump_task(pTask);
	...
	int32_t loop)
{
	...
	/* Traverse forward to adjust
	 * tasks' order according to their priorities
	 */
	for (prev = (cookie % CMDQ_MAX_TASK_IN_THREAD); loop > 0; loop--) {
		...
		if (loop <= 1) {
			...
			CMDQ_MSG("Original PC %x, end %x\n",
				(uint32_t)pPrev->MVABase,
				(uint32_t)(pPrev->MVABase + pPrev->commandSize));
		...
		if (pPrev->priority < pTask->priority) {
			CMDQ_LOG("Switch prev(%d, 0x%p) curr(%d,0x%p) order\n",
					prev,
					pPrev,
					index,
					pTask);
	...
	int32_t thread)
{
	...
	cmdqCoreLongString(false,
		...
		&msgOffset,
		&msgMAXSize,
		"task 0x%p on thr %d VABase: 0x%p, MVABase: %pa,",
		pTask,
		thread,
		pTask->pVABase,
		&(pTask->MVABase));
	...
	if (pThread->taskCount <= 0) {
		...
		enablePrefetch = cmdq_core_should_enable_prefetch(
			pTask->scenario);
		if (enablePrefetch) {
...
#if 0		/*add for debug */
		/* bit 0-2 for priority level; */
		int32_t priority = CMDQ_REG_GET32(CMDQ_THR_CFG(thread)) & 0x07;
		...
		if (priority == threadPrio) {
			...
			CMDQ_LOG("set thread(%d),qos:(%d) failed,qos is %d\n",
		...
		if (pTask->loopCallback) {
			CMDQ_ERR("trigger loop:0x%p on thread %d count=%d\n",
		 ...
		 */
		current_pc = CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread));
		end_pc = CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread));
		if ((CMDQ_AREG_TO_PHYS(current_pc) ==
			(CMDQ_AREG_TO_PHYS(end_pc) - 8)) ||
			(CMDQ_AREG_TO_PHYS(current_pc) ==
			(CMDQ_AREG_TO_PHYS(end_pc) - 0))) {
			longMsg[0] = '\0';
			msgOffset = 0;
			msgMAXSize = CMDQ_LONGSTRING_MAX;
			cmdqCoreLongString(true,
				...
				&msgOffset,
				&msgMAXSize,
				"Set thr(%d) pc from 0x%08x(end:0x%08x) to %pa,",
				thread,
				CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)),
			...
			CMDQ_REG_SET32(CMDQ_THR_END_ADDR(thread),
				CMDQ_PHYS_TO_AREG(
					pTask->MVABase + pTask->commandSize));
			pThread->pCurTask[cookie % CMDQ_MAX_TASK_IN_THREAD] =
				pTask;
			pThread->taskCount++;
			pThread->allowDispatching = 1;
		} else {
			...
			CMDQ_MSG("task in range [%d, %d] with count %d\n",
				minimum,
				cookie,
				loop);

			/* ALPS01672377
			 * .note pThread->taskCount--
			 * when remove task from pThread in ISR
			...
			if (loop < 0) {
				...
				cmdqCoreLongString(true,
					...
					&msgOffset,
					&msgMAXSize,
					"Invalid task count(%d) in thread %d",
					loop,
					thread);
				cmdqCoreLongString(true,
					longMsg,
			...
			if (loop > CMDQ_MAX_TASK_IN_THREAD) {
				CMDQ_LOG("loop = %d, execeed thread max task",
					loop);
			...
			CMDQ_REG_SET32(CMDQ_THR_END_ADDR(thread),
				CMDQ_PHYS_TO_AREG(
					pLast->MVABase + pLast->commandSize));
			pThread->taskCount++;
			pThread->allowDispatching = 1;
		}
				...
				const uint32_t cookie)
{
	...
	if (!pThread->loopCallback) {
#ifdef CONFIG_MTK_FPGA
		/* ASYNC: debug log, use printk_sched
	...
	if (pThread->waitCookie <= cookie) {
		...
		count = (CMDQ_MAX_COOKIE_VALUE - pThread->waitCookie + 1) +
			(cookie + 1);
		CMDQ_ERR("IRQ:waitCookie:%d, hwCookie:%d, count=%d",
			pThread->waitCookie,
			cookie,
			count);
	}

	for (inner = (pThread->waitCookie % CMDQ_MAX_TASK_IN_THREAD);
			count > 0;
			count--, inner++) {
		...
		CMDQ_TIME *pGotIRQ)
{
	...
	if (cmdq_core_get_secure_thread_exec_counter(thread, &cookie) < 0)
		return;
		...
		CMDQ_TIME *pGotIRQ)
{
	...
	CMDQ_MSG("%s, raisedIRQ:0x%08x, shared_cookie(%d, %d, %d)\n",
			__func__,
			raisedIRQ,
			secure_exec_counter[0],
			secure_exec_counter[1],
...
#endif
	if (pThread->pCurTask[cookie % CMDQ_MAX_TASK_IN_THREAD]) {
		...
		CMDQ_ERR("IRQ: can not find task pc:0x%08x, end_pc:0x%08x\n",
			CMDQ_REG_GET32(CMDQ_THR_CURR_ADDR(thread)),
			CMDQ_REG_GET32(CMDQ_THR_END_ADDR(thread)));
		if (pThread->taskCount <= 0) {
			/* suspend HW thread first,
			 * so that we work in a consistent state
			 * outer function should acquire
			 * spinlock - gCmdqExecLock
			 */
			status = cmdq_core_suspend_HW_thread(thread);
			if (status < 0)
			...
			cmdq_core_disable_HW_thread(thread);
			CMDQ_ERR("IRQ: no task for thr(%d)\n",
					thread);
		}
	}
	...
	CMDQ_TIME *pGotIRQ)
{
	...
	if (pThread->loopCallback) {
		...
		cmdq_core_handle_secure_paths_exec_done_notify(thread,
			value,
			pGotIRQ);
	...
	if (loopResult < 0) {
		...
		/* loop CB failed. the EXECUTION
		 * count should not be used as cookie,
...
void cmdqCoreHandleIRQ(int32_t thread)
{
	...
	/* note that do_gettimeofday may cause HWT
	 * in spin_lock_irqsave (ALPS01496779)
	 */
	gotIRQ = sched_clock();
	...
	if (0 == (value & 0x13)) {
		CMDQ_ERR("thread %d got IRQ but flag 0x%08x in NWd\n",
			thread,
			value);
		spin_unlock_irqrestore(&gCmdqExecLock, flags);
		return;
	...
	if (false == cmdq_core_is_a_secure_thread(thread)) {
		...
		if (0 == (enabled & 0x01)) {
			CMDQ_ERR("thread %d got IRQ already disabled 0x%08x\n",
				thread,
				enabled);
			spin_unlock_irqrestore(&gCmdqExecLock, flags);
			return;
...
int32_t cmdqCoreSuspend(void)
{
	...
	if (cmdq_core_can_module_entry_suspend(pEngine) < 0) {
		CMDQ_ERR("MDP running, kill tasks. threads:0x%08x, ref:%d\n",
			execThreads,
			refCount);
		killTasks = true;
	} else if ((refCount > 0) || (0x80000000 & execThreads)) {
		CMDQ_ERR("other running, kill tasks. threads:0x%08x, ref:%d\n",
				execThreads,
				refCount);
		killTasks = true;
	}
	...
	if (killTasks) {
		...
		list_for_each(p, &gCmdqContext.taskActiveList) {
			...
			if (pTask->thread != CMDQ_INVALID_THREAD) {
				...
				/* release all thread and mark
				 * all active tasks as "KILLED"
				 * (so that thread won't release again)
				 */
				CMDQ_ERR("release all threads and HW clocks\n");
...
static void cmdq_core_consume_waiting_list(struct work_struct *_ignore)
{
	...
	list_for_each_safe(p, n, &gCmdqContext.taskWaitList) {
		...
		cmdqCoreLongString(needPrintLog,
			...
			&msgOffset,
			&msgMAXSize,
			"try acquire thread for task: 0x%p, thread_prio: %d,",
			pTask,
			thread_prio);
		cmdqCoreLongString(needPrintLog,
			longMsg,
		...
		if (CMDQ_INVALID_THREAD == thread || NULL == pThread) {
			/* have to wait, remain in wait list */
			CMDQ_MSG("acquire thread fail,wait\n");
			if (needLog) {
				/* task wait too long */
				longMsg[0] = '\0';
				msgOffset = 0;
				msgMAXSize = CMDQ_LONGSTRING_MAX;
				needPrintLog = cmdq_core_should_print_msg(
					LOG_LEVEL_ERR);
				cmdqCoreLongString(needPrintLog,
					longMsg,
					&msgOffset,
					&msgMAXSize,
					"acquire pre-dump for task: 0x%p,",
					pTask);
				cmdqCoreLongString(needPrintLog,
					longMsg,
					&msgOffset,
					&msgMAXSize,
					"prio:%d,task_prio:%d,flag:0x%llx,",
					thread_prio,
					pTask->priority,
					pTask->engineFlag);
				cmdqCoreLongString(needPrintLog,
		...
		CMDQ_MSG("acquire thread w/flag: 0x%llx thread(%d): 0x%p\n",
				pTask->engineFlag,
				thread,
				pThread);
		...
		if (status < 0) {
			CMDQ_ERR("exec_task_async fail, task 0x%p\n",
				pTask);
	...
	if (dumpTriggerLoop) {
		...
		cmdq_core_dump_thread_pc(
			cmdq_core_disp_thread_index_from_scenario(
				CMDQ_SCENARIO_PRIMARY_DISP));
	}

	if (threadAcquired) {
		/* notify some task's SW thread to
		 * change their waiting state.
		 */
				...
				struct TaskStruct **ppTaskOut)
{
	...
	if (pCommandDesc->secData.isSecure) {
		/* start HW thread 15 if it's secure task */
...
int32_t cmdqCoreReleaseTask(struct TaskStruct *pTask)
{
	...
	if (thread != CMDQ_INVALID_THREAD && NULL != pThread) {
		...
		if (pThread->loopCallback) {
			 ...
			 * might meet unsuspendable situation,
			 * can be ignored due to following reset
			 */
			/* status = cmdq_core_suspend_HW_thread(thread); */
			/* WARN_ON(status < 0); */
			pThread->taskCount = 0;
	...
	long timeout_jiffies)
{
	...
	if (pResult && pResult->count) {
		/* clear results */
		memset(CMDQ_U32_PTR(pResult->regValues),
			0,
			pResult->count *
...
static void cmdq_core_auto_release_work(struct work_struct *workItem)
{
	...
	if (pTask) {
		...
		/* Notify user */
		if (finishCallback) {
			CMDQ_VERBOSE("call %p with data 0x%08x\n",
...
void cmdq_core_release_task_by_file_node(void *file_node)
{
	...
	list_for_each(p, &gCmdqContext.taskActiveList) {
		pTask = list_entry(p, struct TaskStruct, listEntry);
		if (pTask->taskState != TASK_STATE_IDLE &&
		pTask->privateData == (cmdqU32Ptr_t)(unsigned long) file_node &&
		(cmdq_core_is_request_from_user_space(pTask->scenario))) {
			CMDQ_LOG("task 0x%p release,file node 0x%p closed\n",
				pTask, file_node);
			 ...
			 * immediately, but we cannot
			 * do so due to SMI hang risk.
			 */
	...
	list_for_each(p, &gCmdqContext.taskWaitList) {
		...
		pTask->privateData == (cmdqU32Ptr_t)(unsigned long) file_node &&
		(cmdq_core_is_request_from_user_space(pTask->scenario))) {
			CMDQ_LOG("WAITING 0x%p release, node 0x%p closed\n",
				pTask,
				file_node);
	...
	const unsigned long long value)
{
	...
	if (value != result) {
		CMDQ_ERR("set_GPR64(%x) failed, value is 0x%llx, not 0x%llx\n",
			regID,
			result,
			value);
	}
...
uint32_t cmdqCoreReadWriteAddress(dma_addr_t pa)
{
	...
	list_for_each(p, &gCmdqContext.writeAddrList) {
		...
		if (offset >= 0 &&
			...
			CMDQ_VERBOSE("%s pa:%pa, offset=%d,va=%p,start=%pa\n",
				__func__,
				&pa,
				offset,
			...
			CMDQ_VERBOSE("%s found offset=%d va=%p value=0x%08x\n",
				__func__,
				offset,
				(pWriteAddr->va + offset),
				value);
...
uint32_t cmdqCoreWriteWriteAddress(dma_addr_t pa, uint32_t value)
{
	...
	list_for_each(p, &gCmdqContext.writeAddrList) {
		...
		/* note it is 64 bit length for
		 * uint32_t variable in 64 bit kernel
		...
		if (offset >= 0 &&
			...
			CMDQ_VERBOSE("%s input:0x%pa, got offset=%d ",
				__func__,
				&pa,
				offset);
			CMDQ_VERBOSE("va=%p pa_start=0x%pa, value=0x%08x\n",
				(pWriteAddr->va + offset),
				&pWriteAddr->pa,
				value);
...
{
#if 0
	/*the caller should ensure value is ileagle not here */
	if (value < LOG_LEVEL_MSG || value >= LOG_LEVEL_MAX) {
...
void cmdq_core_get_clk_map(struct platform_device *pDevice)
{
	...
	for (i = 0; i < CMDQ_CLK_NUM; i++) {
		cmdq_clk_map[i] = devm_clk_get(&pDevice->dev,
			cmdq_core_get_clk_name(i));
...
int cmdq_core_enable_ccf_clk(enum CMDQ_CLK_ENUM clk_enum)
{
	int ret = 0;
...
int cmdq_core_disable_ccf_clk(enum CMDQ_CLK_ENUM clk_enum)
{
	int ret = 0;
...
bool cmdq_core_clock_is_on(enum CMDQ_CLK_ENUM clk_enum)
{
...
#if 0
bool cmdq_core_subsys_is_on(enum CMDQ_SUBSYS_ENUM clk_enum)
{
					...
					struct TaskStruct *pTask)
{
	...
	if (pCommandDesc->secData.isSecure &&
		pCommandDesc->secData.addrMetadataCount) {
		uint32_t size = sizeof(struct cmdqSecAddrMetadataStruct) *
			pCommandDesc->secData.addrMetadataCount;
		pNewBuf = kzalloc(size, GFP_KERNEL);
		if (!pNewBuf) {
			CMDQ_ERR("alloc %d bytes addrMetadatasfailed\n",
				size);
			status = -1;
		}
