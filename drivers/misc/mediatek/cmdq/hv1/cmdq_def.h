...
/* Maximum prefetch buffer size, in instructions. */
...
enum CMDQ_SCENARIO_ENUM {
	 ...
	 * ithout blocking config thread
	 */
	CMDQ_SCENARIO_DISP_SCREEN_CAPTURE = 25,
	/* client from user space, so the cmd buffer is in user space. */
	CMDQ_SCENARIO_USER_MDP = 26,
	/* notifiy there are some tasks exec done in secure path */
	CMDQ_SCENARIO_SECURE_NOTIFY_LOOP = 27,
	...
	CMDQ_SCENARIO_USER_DISP_COLOR = 31,
...
/* Custom "wide" pointer type for
 * 64-bit compatibility. Always cast from uint32_t*.
 */
#define  cmdqU32Ptr_t unsigned long long
//#define cmdqU32Ptr_t unsigned long
...
struct cmdqReadAddressStruct {
	/* [IN] number of entries in result. */
	uint32_t count;
 ...
 * According to handle type, translate handle
 * and replace (_d)th instruciton to
 ...
 *        . use case: OVL reads from secure and
 * normal buffers at the same time)
 */
enum CMDQ_SEC_ADDR_METADATA_TYPE {
	...
	/*memset this specify buffer*/
	CMDQ_SAM_MEMSET_BUFFER = 4,
	/* reverse map normal MVA to secure world */
	CMDQ_SAM_NMVA2_MVA_REVERSE = 5,
};

struct cmdqSecAddrMetadataStruct {
	/* [IN]_d, index of instruction. Update its argB
