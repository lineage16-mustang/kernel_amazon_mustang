...
void cmdq_core_dump_mmsys_config(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(configRegisters); ++i) {
		value = CMDQ_REG_GET16(MMSYS_CONFIG_BASE_VA +
...
int32_t cmdqMdpResetEng(uint64_t engineFlag)
{
	...
	if (engineToResetAgain != 0) {
		...
		CMDQ_ERR("Reset failed(0x%llx), reset again\n",
			 engineToResetAgain);
...
void testcase_clkmgr_mdp(void)
{
	...
	testcase_clkmgr_impl(CMDQ_CLK_DISP0_MDP_TDSHP0,
		...
		0xAACCBBDD, MDP_TDSHP_BASE_VA + 0x244,
		true);

	/* RSZ clk test with debug port */
	testcase_clkmgr_impl(CMDQ_CLK_DISP0_MDP_RSZ0,
		"CMDQ_TEST_MDP_RSZ0",
		MDP_RSZ0_BASE_VA + 0x040,
		0x00000001,
		MDP_RSZ0_BASE_VA + 0x044,
		false);

	testcase_clkmgr_impl(CMDQ_CLK_DISP0_MDP_RSZ1,
		"CMDQ_TEST_MDP_RSZ1",
		MDP_RSZ1_BASE_VA + 0x040,
		0x00000001,
		MDP_RSZ1_BASE_VA + 0x044,
		false);
}
