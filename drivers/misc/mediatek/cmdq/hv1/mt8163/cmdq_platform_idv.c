...
const char *cmdq_core_module_from_event_id(enum CMDQ_EVENT_ENUM event,
	uint32_t instA,
	uint32_t instB)
{
...
#define DECLARE_REG_RANGE(base, name) case base: return #name;
	switch (addr_base_and_page) {
		/* DISPSYS_BASE */
		DECLARE_REG_RANGE(0x14000000, MMSYS_CONFIG);
		/* MDP_RDMA0_BASE */
		DECLARE_REG_RANGE(0x14001000, MDP_RDMA);
		/* MDP_RSZ1_BASE */
		DECLARE_REG_RANGE(0x14002000, MDP_RSZ0);
		/* MDP_WDMA_BASE */
		DECLARE_REG_RANGE(0x14003000, MDP_RSZ1);
		/* MDP_TDSHP_BASE */
		DECLARE_REG_RANGE(0x14004000, MDP_WDMA);
		/* DISP_RDMA_BASE */
		DECLARE_REG_RANGE(0x14005000, MDP_WROT);
		/* DISP_BLS_BASE */
		DECLARE_REG_RANGE(0x14006000, MDP_TDSHP);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x14007000, DISP_OVL0);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x14008000, DISP_OVL1);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x14009000, DISP_RDMA0);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x1400A000, DISP_RDMA1);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x1400B000, DISP_WDMA0);
		/* DSI_BASE */
		DECLARE_REG_RANGE(0x1400C000, DISP_COLOR);
		/* DPI_BASE */
		DECLARE_REG_RANGE(0x1400D000, DISP_CCORR);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x1400E000, DISP_AAL);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x1400F000, DISP_GAMMA);

		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14010000, DISP_DITHER);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14011000, DISP_UFOE);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14012000, DISP_DSI0);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14013000, DISP_DPI0);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14014000, DISP_PWM0);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14015000, MM_MUTEX);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14016000, SMI_LARB0);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14017000, SMI_COMMON);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14018000, DISP_WDMA1);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x14019000, UFOD_RDMA0);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x1401A000, UFOD_RDMA1);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x1401C000, DPI1);
		/* MMSYS_MUTEX_base */
		DECLARE_REG_RANGE(0x1401D000, HDMI);
...
#undef DECLARE_REG_RANGE
	...
	/* for other register address we rely on
	 * GCE subsys to group them with
	 * 16-bit mask.
	 */
...
const int32_t cmdq_core_can_module_entry_suspend(
	struct EngineStruct *engineList)
{
	...
	for (i = 0;
		...
		if (engineList[e].userCount != 0) {
			CMDQ_ERR("suspend but engine %d count %d, owner=%d\n",
				 e,
